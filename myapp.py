...
pressed_emoji = """<span style='font-size: 30px;
                                border-radius: 7px;
                                text-align: center;
                                display:inline;
                                padding-top: 3px;
...
def InitialPage():
    ...
    <ol>
    <li style="font-size:20px";>サイドバーの画像と同じ絵をグリッドから探す</li>
    <li style="font-size:20px";>正解で得点、間違いで-1点</li>
    <li style="font-size:20px";>一定時間ごとに絵が再生成され、再生成ごとに-1点</li>
    <li style="font-size:20px";>ゲーム終了時にスコアがプラスであれば、<strong>勝利</strong>となります。そうでない場合は、<strong>敗北</strong>となります。</li>
    </ol></span>""" 
...
def PressedCheck(vcell):
    if mystate.plyrbtns[vcell]['isPressed'] == False:
        ...
        if mystate.plyrbtns[vcell]['eMoji'] == mystate.sidebar_emoji:
            ...
            if mystate.GameDetails[0] == '簡単': mystate.myscore += 5
            elif mystate.GameDetails[0] == 'ふつう': mystate.myscore += 3
            elif mystate.GameDetails[0] == '難しい': mystate.myscore += 1
...
def PreNewGame():
    ...
    mystate.myscore = 0
    mystate.hint_shown = False
    mystate.hint_cell = None
    mystate.hint_row = None
...
def NewGame():
    ...
    with st.sidebar:
        st.subheader(f"🖼️ 絵合わせ : {mystate.GameDetails[0]}")
        st.markdown(horizontal_bar, True)
        ...
        if aftimer > 0:
            mystate.myscore -= 1
            if not mystate.hint_shown:
                mystate.hint_shown = True

        st.info(f"{ScoreEmoji()} スコア: {mystate.myscore} | 残り: {(total_cells_per_row_or_col ** 2)-len(mystate.expired_cells)}")

        st.markdown(horizontal_bar, True)
        if st.button(f"🔙 戻る", use_container_width=True):
    ...
    for i in range(1, (total_cells_per_row_or_col+1)):
        if mystate.hint_shown and mystate.hint_row == i:
            st.markdown("<div style='background:#fff7d0; padding:0.35rem 0.5rem; border-radius:0.6rem; margin-bottom:0.4rem;'>💡 ヒント行: この行のどこかに答えの画像があります。</div>", unsafe_allow_html=True)
...
def Main():
    ...
    InitialPage()
    with st.sidebar:
        mystate.GameDetails[0] = st.radio('難易度:', options=('簡単', 'ふつう', '難しい'), index=1, horizontal=True, )
        
        if st.button(f"🕹️ スタート", use_container_width=True):
